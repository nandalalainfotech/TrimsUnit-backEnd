...
export class SupplierreportService {
    ...
    async findAll(): Promise<Supplierreport001wb[]> {
        return await this.SupplierreportRepository.find({relations: ["supregslNo2", "activeslNo2", "supcheckslNo2"]});
    }

    async findAllBySupplierId(supregslno: number): Promise<Supplierreport001wb[]> {
        return this.SupplierreportRepository.find({ relations: ["supregslNo2","activeslNo2", "supcheckslNo2"], where: { "supregslNo": supregslno } });
    }
