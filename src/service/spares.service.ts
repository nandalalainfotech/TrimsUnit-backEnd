...
export class SpareService {
    ...
    async findAll(): Promise<Spares001mb[]> {
        return await this.spareRepository.find({ relations: ["msslno2","sslno2"] });
    }
    ...
    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        let spares = await this.spareRepository.find({ relations: ["msslno2","sslno2"] });
        var fs = require('fs');
        var pdf = require('dynamic-html-pdf');
