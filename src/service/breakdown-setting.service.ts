...
export class BreakdownService {
    ...
    async findAll(): Promise<Breakdown001mb[]> {
        return await this.breakdownRepository.find({ relations: ["sslno2"] });
    }
    ...
    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        let breaks = await this.breakdownRepository.find({ relations: ["sslno2"] });
        var fs = require('fs');
        var pdf = require('dynamic-html-pdf');
