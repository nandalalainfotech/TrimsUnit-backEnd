...
export class FixtureSettingService {
    ...
    async findAll(): Promise<Fixture001mb[]> {
        return this.fixtureRepository.find({ relations: ["sslno2"] });
    }

    async findAllSlNoAndMcode(): Promise<Fixture001mb[]> {
        return this.fixtureRepository.find({select : ['slNo', 'fcode','fname']});
    }
    ...
    async downloadExcel(@Req() request: Request, @Res() response: Response) {
        let machine = await this.fixtureRepository.find({  relations: [ "sslno2", ],});
