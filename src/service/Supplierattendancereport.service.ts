...
export class SupplierattendanceService {
    ...
    async findAllBySupplierId(supregslNo: number): Promise<Supplierattendancereport001wb[]> {
        return this.SupplierattendanceRepository.find({ relations: ["supregslNo2", "trainingslNo2"], where: { "supregslNo": supregslNo } });
    }

    async findAll(): Promise<Supplierattendancereport001wb[]> {
        return await this.SupplierattendanceRepository.find({ relations: ["supregslNo2", "trainingslNo2"] });
    }
