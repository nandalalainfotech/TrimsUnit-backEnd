...
export class ChecklistSettingService {
    ...
    async findAll(): Promise<Checklist001mb[]> {
        return await this.checklistRepository.find({ relations: ["mcslno2","sslno2"] });
    }
    ...
    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        let checklist = await this.checklistRepository.find({ relations: ["mcslno2","sslno2"] });
        var fs = require('fs');
        var pdf = require('dynamic-html-pdf');
