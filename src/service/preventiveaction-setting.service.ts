...
export class PreventiveactionService {
    ...
    async findAll(): Promise<Preventiveaction001mb[]> {
        return await this.PreventiveactionRepository.find({ relations: ["sslno2", "rcslno2"] });
    }

    async findAllbyRootCauseId(rcslno: number): Promise<Preventiveaction001mb[]> {
        return this.PreventiveactionRepository.find({ select: ["slNo","name"], where: { "rcslno": rcslno } });
    }
    ...
    async downloadPdf(@Req() request: Request, @Res() response: Response) {
        let preventive = await this.PreventiveactionRepository.find({ relations: ["sslno2","rcslno2"] });
        var fs = require('fs');
        var pdf = require('dynamic-html-pdf');
