...
export class DailyCheckListService {
    ...
    async findAll(): Promise<Dailychecklist001wb[]> {
        return await this.dailycheckRepository.find({ relations: ["mcslno2",] });
    }

    async findAllByMachineId(mslno: number): Promise<Dailychecklist001wb[]> {
        return this.dailycheckRepository.find({ relations: ["mslno2", "cpslno2"], where: { "mslno": mslno } });
    }
