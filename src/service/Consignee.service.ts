...
export class ConsigneeService {
    ...
    async findAllbyPurchaseOrderId(poslNo: number): Promise<Consignee001mb[]> {
        return this.consigneeRepository.find({ select: ["slNo","consignee"], where: { "companySlno": poslNo } });
    }
    ...
    async findAll(): Promise<Consignee001mb[]> {
        return await this.consigneeRepository.find({relations: ["companySlno2"]});
    }
...
async downloadPdf(@Req() request: Request, @Res() response: Response) {        
        let consignee = await this.consigneeRepository.find({relations: ["companySlno2"]});
