...
export class MachineSettingService {
    ...
    async findAll(): Promise<Machine001mb[]> {
        return this.machineRepository.find({ relations: ["sslno2"] });
    }

    async findAllSlNoAndMcode(): Promise<Machine001mb[]> {
        return this.machineRepository.find({select : ['slNo', 'mcode','mname']});
    }
    ...
    async downloadExcel(@Req() request: Request, @Res() response: Response) {
        let machine = await this.machineRepository.find({  relations: [ "sslno2", ],});
