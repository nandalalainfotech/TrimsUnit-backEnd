...
export class StatutoryPlanService {
    ...
    // async findNotificationAll(): Promise<Statutory001wb[]> {
    //     const start = new Date();
    //     const end = new Date();
    //     end.setDate(end.getDate() +45)
    //     const result = await this.statuplanRepository.find({
    //         relations: ["emslno2"],
    //         where: {
    //             date: Between(
    //                 new Date(start),
    ...
    //     return result;
    // }
    async findAll(): Promise<Statutory001wb[]> {
        return this.statuplanRepository.find({ relations: ["bslno2"] });
    }
    
    // async findAllByEmployeNameId(bslno: number): Promise<Statutory001wb[]> {
    //     return this.statuplanRepository.find();
    // }
