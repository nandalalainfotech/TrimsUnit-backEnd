...
export class CompanyService {
    ...
    async findAll(): Promise<Companydetails001mb[]> {
        return await this.companyRepository.find();
    }

    async findAllBycompany(companySlno: number): Promise<Companydetails001mb[]> {
        return this.companyRepository.find({ relations: ["companySlno2"],
...
async downloadPdf(@Req() request: Request, @Res() response: Response) {        
        let companydetails = await this.companyRepository.find();
