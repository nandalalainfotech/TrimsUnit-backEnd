...
export class SupplierContactService {
    ...
    async findAll(): Promise<Suppliercontact001wb[]> {        
        return this.SupplierContactRepository.find()
    }

    async findAllbysupplierId(poslNo: number): Promise<Suppliercontact001wb[]> {
        return this.SupplierContactRepository.find({ select: ["slNo","pname"], where: { "supplierslNo": poslNo } });
    }
