...
export class PreventiveCheckListService {
    ...
    async findAll(): Promise<Preventivechecklist001wb[]> {
        return this.preCheckRepository.find({ relations: ["mslno2", "cpslno2"] });
    }

    async findAllByMachineId(mslno: number): Promise<Preventivechecklist001wb[]> {
        return this.preCheckRepository.find({ relations: ["mslno2", "cpslno2"], where: { "mslno": mslno } });
    }
